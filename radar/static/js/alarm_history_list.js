...
//         var Table = ge(this.Id_Table),
//             i=0, row_temp, cells, z;
//         var el_tbody = document.createElement('tbody');
//             el_tbody.id = 'id_Alarm_List_Tbody';
//
//         if (y != null) {
...
//
//
//             for (i=0; i<Table.CellsLenght; i++){
//                 var c = Table.RowHead.cells[i];
...
//             if (Table.p == y) {
//                 Table.a.reverse();
//                 x.className = Table.Orientation ? this.asc : this.desc;
//                 Table.Orientation = Table.Orientation ? 0 : 1
//             }
//             else {
//                 Table.p = y;
//                 console.info('dddd');
//                 alarm_history_db.change_data_arr(y,"ASC");
//                 Table.Orientation = 0;
...
//                     }
//                 }
//                 Table.replaceChild(el_tbody, Table.b);
//                 if (this.paginate) {
...
//                         continue;
//                     }
//                     if ((Table.RowHead.cells[ii].className=="asc")||(Table.RowHead.cells[ii].className=="desc")) {
//                         sortNum=ii;
//                     }
//                 }
//
...
//         if (this.currentid&&this.limitid) {
//             T$(this.limitid).innerHTML = this.Orientation;
//         }
//     };
//
//     sorter.prototype.pages = function() {
//         this.Orientation = Math.ceil(this.RowsLenght/this.pagesize)
//     };
//
//     function ge(e) {
//         var tabl = T$(e);
...
var alarm_history_tbl = new function () {
    ...
    this.change_row_arr = function (fl_highlight, tbl_arr, orientation) {
        ...
        if (tbody.children.length > rowNum) {
            end_row = tbody.children.length;
            for (i=rowNum; i<end_row; i++) {
                tbody.removeChild(tbody.children[tbody.children.length-1]);
            }
        }
        if (tbody.children.length > 20) {
            end_row = tbody.children.length;
            for (i=20; i<end_row; i++) {
                tbody.removeChild(tbody.children[tbody.children.length-1]);
            }
        }
        if (Count_Alarm_History_Row_DB) {
